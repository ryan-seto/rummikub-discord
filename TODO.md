Bugs:
- Attempted to fix: end turn undo's everything even if the board is in a valid state need to update that
- if you havent satisfied the 30 point rule you shouldn't be able to manipulate the board, also 30 point just doesn't work
- Attempted to fix: mid-game, if you haven't played a tile and you manipulate the board, the undo actions don't enable only draw tile is allowed which isn't right
